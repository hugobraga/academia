...
#ifdef USE_CHANNEL1
#define LOCAL_PROCESS_IP1   "192.168.1.2"
#define OTHER_PROCESS_IP1   "192.168.1.2"
#define PROCESS_PORT1       5010
#endif

#ifdef USE_CHANNEL2
#define LOCAL_PROCESS_IP2   "192.168.149.22"
...
int notify_timely_channel(struct sockaddr_in p1_addr, struct sockaddr_in p2_addr, qosp_channel_t *channel) {
    ...
    //printf("antes de notifyNewRouter\n");
    //sleep(1);
    //exit(0);
    notifyNewRouter(router_addr, qospA_addr, &router_desc);
    //printf("depois de notifyNewRouter\n");
    //sleep(1);
    //exit(0);
    notifyNewChannel(p1_addr, p2_addr, channel);
    //printf("depois de notify_timely_channel\n");
    //sleep(1);
    //exit(0);
    ...
    cout << "vai notificar que um canal eh timely\n";
    channel->listener_desc = notifyTimelyChannel(*channel);
    //cout << "depois notifyTimelyChannel(*channel)\n";
}
...
int run_app_process() {
    ...
    //other_addr = addr;
    //printf("antes de notify_timely_channel\n");
    //sleep(1);
    //exit(0);
    cout << "vai notificar canal timely\n";
...
int main(int argc, char** argv) {
    ...
    char *ips_qospA[1] = {(char *)"10.10.20.2"};
    #ifdef HOLISTIC_QoSP
    ...
    if (!rt_task_shadow(&MAIN, NULL, 98, TASK_MODE))
        cout << "a tarefa eh de tempo real\n";
    else
        cout << "nao eh de tempo real\n";
    ...
    //printf("ai\n");
    //exit(0);
    //printf("antes de run_app_process\n");
    //sleep(1);
    //exit(0);
    cout << "vai executar run_app_process\n";
    run_app_process();
    #endif
    #endif
